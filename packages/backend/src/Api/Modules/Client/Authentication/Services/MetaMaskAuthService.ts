...
class MetaMaskAuthService {
  public async findOrCreateAuthAccount(findOrCreateAuthAccountDto: FindOrCreateAuthAccountDto) {
    const { auth_provider, userId, queryRunner } = findOrCreateAuthAccountDto;
    
    const user_id = JSON.parse(userId);
    
    const authAccountRepository: Repository<AuthAccount> = queryRunner.manager.getRepository(AuthAccount);
  ...
  public async verifySignature(address: string, signature: string, queryRunner: QueryRunner) {
    const authAccountRepository: Repository<AuthAccount> = queryRunner.manager.getRepository(AuthAccount);
    ...
    const decodedAddress = ethers.verifyMessage(parsedUser.nonce.toString(), signature);
